...
def process(line, dict2, search_items):
    ...
    if (none_available or hdtv or bdrip or non_english or 
        dvdrip or xvid or divx or webrip or webdl):
        return
    ...
    try:
        ...
        for term in search_terms:
            if term in title:
                search_items[displayed_title] = "{} {}, /msg {} xdcc send {}".format(size, number, user, slot)
...
def main():
    ...
    keys.sort()
    output_file = "hexchat_output_search_items.txt"
    with open(output_file, "w", encoding="utf8") as outfile:        
        for key in keys:
            
            try:
                outfile.write(key + " " + search_items[key] + "\n")
